    ...
    public class PagamentosExecutadosRepository : IPagamentosExecutadosRepository
    {
        ...
        public ExecucaoOrcamentalListagemResponse GetExecucaoOrcamental(RelatorioExecucaoOrcamentalListagemRequest request)
        {
            ...
            var contas = listaContas
                ...
                .Select(e => new ExecucaoOrcamentalDataContract()
                {
                    ...
                    // Soma dos pagamentos executados de janeiro (por DataObrigacao; sem DataObrigacao usa DataCriacao como fallback para não perder histórico)
                    janeiro = e.ComponentedespesaRegisto.Select(x => x.Compromisso.SelectMany(a => a.Pagamentosexecutados.Where(s => s.IndActivo && (s.DataObrigacao ?? s.DataCriacao).Month == 1 && (s.DataObrigacao ?? s.DataCriacao).Year == request.year)).Sum(s => s.ValorExecutado)).Sum(),
                    // Soma dos pagamentos executados de fevereiro
                    fevereiro = e.ComponentedespesaRegisto.Select(x => x.Compromisso.SelectMany(a => a.Pagamentosexecutados.Where(s => s.IndActivo && (s.DataObrigacao ?? s.DataCriacao).Month == 2 && (s.DataObrigacao ?? s.DataCriacao).Year == request.year)).Sum(s => s.ValorExecutado)).Sum(),
                    // Soma dos pagamentos executados de março
                    marco = e.ComponentedespesaRegisto.Select(x => x.Compromisso.SelectMany(a => a.Pagamentosexecutados.Where(s => s.IndActivo && (s.DataObrigacao ?? s.DataCriacao).Month == 3 && (s.DataObrigacao ?? s.DataCriacao).Year == request.year)).Sum(s => s.ValorExecutado)).Sum(),
                    // Soma dos pagamentos executados de abril
                    abril = e.ComponentedespesaRegisto.Select(x => x.Compromisso.SelectMany(a => a.Pagamentosexecutados.Where(s => s.IndActivo && (s.DataObrigacao ?? s.DataCriacao).Month == 4 && (s.DataObrigacao ?? s.DataCriacao).Year == request.year)).Sum(s => s.ValorExecutado)).Sum(),
                    // Soma dos pagamentos executados de maio
                    maio = e.ComponentedespesaRegisto.Select(x => x.Compromisso.SelectMany(a => a.Pagamentosexecutados.Where(s => s.IndActivo && (s.DataObrigacao ?? s.DataCriacao).Month == 5 && (s.DataObrigacao ?? s.DataCriacao).Year == request.year)).Sum(s => s.ValorExecutado)).Sum(),
                    // Soma dos pagamentos executados de junho
                    junho = e.ComponentedespesaRegisto.Select(x => x.Compromisso.SelectMany(a => a.Pagamentosexecutados.Where(s => s.IndActivo && (s.DataObrigacao ?? s.DataCriacao).Month == 6 && (s.DataObrigacao ?? s.DataCriacao).Year == request.year)).Sum(s => s.ValorExecutado)).Sum(),
                    // Soma dos pagamentos executados de julho
                    julho = e.ComponentedespesaRegisto.Select(x => x.Compromisso.SelectMany(a => a.Pagamentosexecutados.Where(s => s.IndActivo && (s.DataObrigacao ?? s.DataCriacao).Month == 7 && (s.DataObrigacao ?? s.DataCriacao).Year == request.year)).Sum(s => s.ValorExecutado)).Sum(),
                    // Soma dos pagamentos executados de agosto
                    agosto = e.ComponentedespesaRegisto.Select(x => x.Compromisso.SelectMany(a => a.Pagamentosexecutados.Where(s => s.IndActivo && (s.DataObrigacao ?? s.DataCriacao).Month == 8 && (s.DataObrigacao ?? s.DataCriacao).Year == request.year)).Sum(s => s.ValorExecutado)).Sum(),
                    // Soma dos pagamentos executados de setembro
                    setembro = e.ComponentedespesaRegisto.Select(x => x.Compromisso.SelectMany(a => a.Pagamentosexecutados.Where(s => s.IndActivo && (s.DataObrigacao ?? s.DataCriacao).Month == 9 && (s.DataObrigacao ?? s.DataCriacao).Year == request.year)).Sum(s => s.ValorExecutado)).Sum(),
                    // Soma dos pagamentos executados de outubro
                    outubro = e.ComponentedespesaRegisto.Select(x => x.Compromisso.SelectMany(a => a.Pagamentosexecutados.Where(s => s.IndActivo && (s.DataObrigacao ?? s.DataCriacao).Month == 10 && (s.DataObrigacao ?? s.DataCriacao).Year == request.year)).Sum(s => s.ValorExecutado)).Sum(),
                    // Soma dos pagamentos executados de novembro
                    novembro = e.ComponentedespesaRegisto.Select(x => x.Compromisso.SelectMany(a => a.Pagamentosexecutados.Where(s => s.IndActivo && (s.DataObrigacao ?? s.DataCriacao).Month == 11 && (s.DataObrigacao ?? s.DataCriacao).Year == request.year)).Sum(s => s.ValorExecutado)).Sum(),
                    // Soma dos pagamentos executados de dezembro
                    dezembro = e.ComponentedespesaRegisto.Select(x => x.Compromisso.SelectMany(a => a.Pagamentosexecutados.Where(s => s.IndActivo && (s.DataObrigacao ?? s.DataCriacao).Month == 12 && (s.DataObrigacao ?? s.DataCriacao).Year == request.year)).Sum(s => s.ValorExecutado)).Sum()
                })
                .ToList();
        ...
        public ClassificacaoEconomicaExecucaoListagemResponse GetExecucaoOrcamentalPorClassificacaoEconomica(RelatorioClassificacaoEconomicaRequest request)
        {
            ...
            var despesaPorConta = allNodes.Values
                ...
                {
                    AgrupamentoId = a.Id,
                    Janeiro = a.ComponentedespesaRegisto.Where(x => x.InstitutionId == request.institution).Select(x => x.Compromisso.SelectMany(c => c.Pagamentosexecutados.Where(p => p.IndActivo && (p.DataObrigacao ?? p.DataCriacao).Month == 1 && (p.DataObrigacao ?? p.DataCriacao).Year == request.year)).Sum(p => p.ValorExecutado)).Sum(),
                    Fevereiro = a.ComponentedespesaRegisto.Where(x => x.InstitutionId == request.institution).Select(x => x.Compromisso.SelectMany(c => c.Pagamentosexecutados.Where(p => p.IndActivo && (p.DataObrigacao ?? p.DataCriacao).Month == 2 && (p.DataObrigacao ?? p.DataCriacao).Year == request.year)).Sum(p => p.ValorExecutado)).Sum(),
                    Marco = a.ComponentedespesaRegisto.Where(x => x.InstitutionId == request.institution).Select(x => x.Compromisso.SelectMany(c => c.Pagamentosexecutados.Where(p => p.IndActivo && (p.DataObrigacao ?? p.DataCriacao).Month == 3 && (p.DataObrigacao ?? p.DataCriacao).Year == request.year)).Sum(p => p.ValorExecutado)).Sum(),
                    Abril = a.ComponentedespesaRegisto.Where(x => x.InstitutionId == request.institution).Select(x => x.Compromisso.SelectMany(c => c.Pagamentosexecutados.Where(p => p.IndActivo && (p.DataObrigacao ?? p.DataCriacao).Month == 4 && (p.DataObrigacao ?? p.DataCriacao).Year == request.year)).Sum(p => p.ValorExecutado)).Sum(),
                    Maio = a.ComponentedespesaRegisto.Where(x => x.InstitutionId == request.institution).Select(x => x.Compromisso.SelectMany(c => c.Pagamentosexecutados.Where(p => p.IndActivo && (p.DataObrigacao ?? p.DataCriacao).Month == 5 && (p.DataObrigacao ?? p.DataCriacao).Year == request.year)).Sum(p => p.ValorExecutado)).Sum(),
                    Junho = a.ComponentedespesaRegisto.Where(x => x.InstitutionId == request.institution).Select(x => x.Compromisso.SelectMany(c => c.Pagamentosexecutados.Where(p => p.IndActivo && (p.DataObrigacao ?? p.DataCriacao).Month == 6 && (p.DataObrigacao ?? p.DataCriacao).Year == request.year)).Sum(p => p.ValorExecutado)).Sum(),
                    Julho = a.ComponentedespesaRegisto.Where(x => x.InstitutionId == request.institution).Select(x => x.Compromisso.SelectMany(c => c.Pagamentosexecutados.Where(p => p.IndActivo && (p.DataObrigacao ?? p.DataCriacao).Month == 7 && (p.DataObrigacao ?? p.DataCriacao).Year == request.year)).Sum(p => p.ValorExecutado)).Sum(),
                    Agosto = a.ComponentedespesaRegisto.Where(x => x.InstitutionId == request.institution).Select(x => x.Compromisso.SelectMany(c => c.Pagamentosexecutados.Where(p => p.IndActivo && (p.DataObrigacao ?? p.DataCriacao).Month == 8 && (p.DataObrigacao ?? p.DataCriacao).Year == request.year)).Sum(p => p.ValorExecutado)).Sum(),
                    Setembro = a.ComponentedespesaRegisto.Where(x => x.InstitutionId == request.institution).Select(x => x.Compromisso.SelectMany(c => c.Pagamentosexecutados.Where(p => p.IndActivo && (p.DataObrigacao ?? p.DataCriacao).Month == 9 && (p.DataObrigacao ?? p.DataCriacao).Year == request.year)).Sum(p => p.ValorExecutado)).Sum(),
                    Outubro = a.ComponentedespesaRegisto.Where(x => x.InstitutionId == request.institution).Select(x => x.Compromisso.SelectMany(c => c.Pagamentosexecutados.Where(p => p.IndActivo && (p.DataObrigacao ?? p.DataCriacao).Month == 10 && (p.DataObrigacao ?? p.DataCriacao).Year == request.year)).Sum(p => p.ValorExecutado)).Sum(),
                    Novembro = a.ComponentedespesaRegisto.Where(x => x.InstitutionId == request.institution).Select(x => x.Compromisso.SelectMany(c => c.Pagamentosexecutados.Where(p => p.IndActivo && (p.DataObrigacao ?? p.DataCriacao).Month == 11 && (p.DataObrigacao ?? p.DataCriacao).Year == request.year)).Sum(p => p.ValorExecutado)).Sum(),
                    Dezembro = a.ComponentedespesaRegisto.Where(x => x.InstitutionId == request.institution).Select(x => x.Compromisso.SelectMany(c => c.Pagamentosexecutados.Where(p => p.IndActivo && (p.DataObrigacao ?? p.DataCriacao).Month == 12 && (p.DataObrigacao ?? p.DataCriacao).Year == request.year)).Sum(p => p.ValorExecutado)).Sum(),
                })
