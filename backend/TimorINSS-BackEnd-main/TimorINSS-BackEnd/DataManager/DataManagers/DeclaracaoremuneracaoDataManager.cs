    ...
    public class DeclaracaoremuneracaoDataManager : IDeclaracaoremuneracaoDataManager
    {
        ...
        public GetDeclaracaoByEntidadeAndFilterResponse GetDeclaracaoByEntidadeAndFilter(GetDeclaracaoByEntidadeAndFilterRequest request)
        {
            ...
            else
            {
                ...
                int decodedId = IdDecoder.DecodeId(request.IdEntidadeStr);
        ...
        public SituacaoContributivaEmpresasRelatorioResponse GetSituacaoContributivaEmpresasRelatorio(SituacaoContributivaEmpresasRelatorioRequest request)
        {
            ...
            try
            {
                response = _unitOfWork.DeclaracaoRemuneracaoRepository.GetSituacaoContributivaEmpresasRelatorio(request);
            }
            catch (Exception e)
            {
        ...
        public ContribuicoesTrendsRelatorioResponse GetContribuicoesTrendsRelatorio(ContribuicoesTrendsRelatorioRequest request)
        {
            ...
            try
            {
                ...
                var contribuicoesPorMes = _unitOfWork.DeclaracaoRemuneracaoRepository.GetContribuicoesTrendsPorMes(beginDate, endDate);
                var registosPorMes = _unitOfWork.EntidadeEmpregadoraRepository.GetNovosRegistosPorMes(beginDate, endDate);

                var meses = contribuicoesPorMes
                    .Select(c => new ContribuicoesTrendMesDataContract
                    {
                        mesAno = c.mesAno,
                        valorPago = c.valorPago,
                        valorDivida = c.valorDivida,
                        novosRegistos = registosPorMes.Where(r => r.mesAno == c.mesAno).Select(r => r.novosRegistos).FirstOrDefault()
                    })
                    .ToList();

                // Meses com novos registos mas sem nenhuma declaração ainda (empresa muito recente)
                foreach (var registo in registosPorMes)
                {
                    if (!meses.Any(m => m.mesAno == registo.mesAno))
                        meses.Add(new ContribuicoesTrendMesDataContract { mesAno = registo.mesAno, novosRegistos = registo.novosRegistos });
                }

                response.meses = meses.OrderBy(m => m.mesAno).ToList();
            }
            catch (Exception e)
            {
                response.Errors.Add(new Error { ErrorCode = "-1", ErrorMessage = e.Message });
