...
export class ExcelImporterComponent implements OnInit {
  ...
  constructor(public translate: TranslateService,
    ...
    private importerToastsService: ImporterToastsService,
  ...
  public submit(files: FileList) {
    ...
    this.http.post<any>(`${environment.apiUrl}${this.url}`, formData)
    .toPromise()
    .then(response => {
      this.importerToastsService.add(response.id, this.onComplete, this.type);
    })
    .catch(err => {
      const errors = err?.error?.errors ? err.error.errors.map((x: any) => x.errorCode) : ['-1'];
  ...
  public showError(errors: string[]) {
    openErrorsDialog(errors, this.errorDialog);
    this.spinner.hide();
  }
  ...
  public downloadExcel() {
    ...
    var csv = 'sep=,\r\n' + this.columns.join(',');
    var data = new Blob([new Uint8Array([0xEF,0xBB,0xBF]), csv]);
    a.href = URL.createObjectURL(data);
    a.download = this.getResolvedText(this.text) + '.csv';
    a.click();
  }

  private getResolvedText(text: string) {
    return this.removeDiacritics(text.toLowerCase()).replace(/ /g,'-');
  }
