...
// nhập liệu AD/Cabimento — không cần API mới. CHỈ XEM.
@Component({
  selector: 'app-registo-ad',
  templateUrl: './registo-ad.component.html',
  styleUrls: ['./registo-ad.component.css']
})
export class RegistoAdComponent implements OnInit {

  public loading = false;
  public items: ExpenditureAuthorizationDataContract[] = [];
  public pagedItems: ExpenditureAuthorizationDataContract[] = [];
  public pageIndex = 0;
  public pageSize = 20;
  ...
  public estadoOptions: { value: string; label: string }[] = [
    { value: '', label: 'registoAd.estadoTodos' },
    { value: 'DRAFT', label: 'registoAd.estadoDraft' },
    { value: 'PENDING_REVIEW', label: 'registoAd.estadoPendingReview' },
    { value: 'PENDING_APPROVAL', label: 'registoAd.estadoPendingApproval' },
    { value: 'APPROVED', label: 'registoAd.estadoApproved' }
  ];
  ...
  ngOnInit(): void {
    ...
    this.organizationSearch = this.translate.instant('registoAd.bothOption');
  ...
  public selectOrganization(o?: SelectDescription): void {
    ...
    this.organizationSearch = o ? o.nome : this.translate.instant('registoAd.bothOption');
    this.applyFilters();
  }
  ...
  public load(): void {
    this.loading = true;
    this.expenditureAuthorizationService.getByAno(this.selectedYear).subscribe(
      response => {
        this.loading = false;
  ...
  private allItems: ExpenditureAuthorizationDataContract[] = [];
  ...
  public get totalValorAutorizado(): number {
    return this.items.reduce((sum, i) => sum + (i.valorAutorizado ?? 0), 0);
  }

  public get totalValorCabimentado(): number {
    return this.items.reduce((sum, i) => sum + (i.valorCabimentado ?? 0), 0);
  }
