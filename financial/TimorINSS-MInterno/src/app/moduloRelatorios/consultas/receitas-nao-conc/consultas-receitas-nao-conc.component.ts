...
import { ReceitaNaoConciliadaRelatorio, ReceitaRelatorio } from "src/app/models/componenteReceitaRegisto";
...
import { ReceitasNaoConciliadasRelatorioRequest } from "src/app/request-models/componenteReceitaRegisto-request";
...
export class ConsultasReceitasNaoConciliadasComponent implements OnInit {
  ...
  public getReceitasTable() {
    ...
    this.componenteReceitaRegistoService.GetReceitasNaoConciliadasRelatorio(request).subscribe((receitas) => {
      this.receitasList = receitas.receitas || [];
      this.totalRowsTable = receitas.rows;
      this.totais.valor = receitas.receitas.reduce((inc: number, receita: ReceitaNaoConciliadaRelatorio) => inc + receita.valor, 0);
  ...
  public exportExcelRelatorio() {
    ...
    this.componenteReceitaRegistoService.GetReceitasNaoConciliadasRelatorioExcel(request).subscribe((response) => {
      ...
      blobExcelSaveAs(response.file, `consultation-unreconciled-revenues${date}`);
    },
      err => {
        this.hideLoader();
        err.error?.errors ? err.error.errors.map((x: any) => this.errors.push(x.errorCode)) : this.errors.push('-1');
