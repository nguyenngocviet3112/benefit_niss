...
/**
 * Execução por Programa — relatório de execução orçamental consolidado (por instituição/conta OGE),
 * reutilizando os mesmos endpoints GetExecucaoOrcamental/GetExecucaoOrcamentalExcel do relatório
 * "Execução Orçamental" já existente. Ecrã próprio (rota/menu separado) para não sobrepor o relatório antigo.
 * (Nota: este ecrã chamava-se "CE_OSS_Global" antes de 2026-07-18 -- foi renomeado porque o nome real
 * "CE_OSS_Global" passou a designar o relatório por Classificação Económica, ver classificacao-economica/.)
 */
@Component({
  selector: 'app-reports-execucao-por-programa',
  templateUrl: './reports-execucao-por-programa.component.html',
  styleUrls: ['./reports-execucao-por-programa.component.css']
})
export class RelatoriosExecucaoPorProgramaComponent implements OnInit {
  ...
  public exportExcelRelatorio() {
    ...
    (isReceita ? this.receitaRegistoService.GetExecucaoOrcamentalExcel(request) : this.pagamentosService.GetExecucaoOrcamentalExcel(request)).subscribe((response) => {
      this.hideLoader();
      blobExcelSaveAs(response.file, `Execucao_Por_Programa-${this.year}`);
    },
      err => {
        this.hideLoader();
        err.error?.errors ? err.error.errors.map((x: any) => this.errors.push(x.errorCode)) : this.errors.push('-1');
